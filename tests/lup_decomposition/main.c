...
#define n 4
	...
	int pA[n];
	lup_decomposition_decompose(GA, pA, n);

	double detA = lup_decomposition_determinant(GA, pA, n);
	printf("det(A) = %f\n", detA);
	...
	int pB[n];
	lup_decomposition_decompose(GB, pB, n);

	double detB = lup_decomposition_determinant(GB, pB, n);
	printf("det(B) = %f\n", detB);
