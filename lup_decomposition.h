...
// It returns the nuwber of rows interchanged and will be used later in the calculation of the sign of the determinant
int lup_decomposition_decompose(float* A, int* p, int n);
...
// The variable num_swaps specifies the number of row interchanges performed in the procedure lup_decomposition_decompose
double lup_decomposition_determinant(const float* A, const int* p, int n, int num_swaps);
...
double lup_decomposition_determinant(const float* A, const int* p, int n, int num_changes)
{
	// Sign according to permutation.
	unsigned int sign = -1.0;
...
//		}
	...
	return (pow(sign, num_changes) * det);
}
